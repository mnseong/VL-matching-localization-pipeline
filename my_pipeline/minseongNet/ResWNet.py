...
class ResWNet(nn.Module):
    def __init__(self,
        ...
        self.upconv1    = upconv(256, 128, 3, 2)
        self.upconv0    = upconv(128, 64, 3, 2)
    ...
    def forward(self, x):
        ...
        final = torch.cat([a_fusion, b_fusion], dim=1)
        final = self.cbam1(final)
        # print(final.shape)
        final = self.upconv1(final)
        # print(final.shape)
        # final = conv(256, 128, 128, 128)
        final = self.cbam2(final)
        final = self.upconv0(final)
        # print(final.shape)
        final = self.output(final)
        # print(final.shape)

        # print("이건 fusion")
        # print(a_fusion.shape)
        ...
        # print("이건 fusion_cn")
        # print(a_fusion_cn.shape)

        # Measure peakiness score a
        # a_pf = self.peakiness_score(a_fusion_cn)
        # a_mps = self.relu(self.norm1(self.conv1(a_pf*a_fusion_cn)))
        #
        # a_cbam = self.cbam(a_mps)


        # Measure peakiness score b
        # b_pf = self.peakiness_score(b_fusion_cn)
        # b_mps = self.relu(self.norm1(self.conv1(b_pf*b_fusion_cn)))
        #
        # b_cbam = self.cbam(b_mps)
        ...
        # final = torch.cat([a_cbam, b_cbam], dim=1)
